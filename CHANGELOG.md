...
- **Formal memory entity types taxonomy** (#48) — `VALID_MEMORY_TYPES` constant with 7 types: `decision`, `pattern`, `outcome`, `feedback`, `blocker`, `requirement`, `context`. `mem0_store` validates type and returns error with list of valid types on invalid input. `mem0_recall_context` accepts optional `memory_types` param (comma-separated) for type-filtered retrieval via Qdrant `should` conditions. Backward compatible — old memories with other types remain readable.
